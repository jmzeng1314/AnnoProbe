...
phenoDat <- pData(eSet)
head(phenoDat[,1:4])
group_list=substring(phenoDat$title,nchar(phenoDat$title))

## check GPL and annotate the probes to genes.
(gpl=eSet@annotation)
checkGPL(gpl)
printGPLInfo(gpl)
probe2gene=idmap(gpl='GPL13607',type = 'pipe')
head(probe2gene)
...
check_diff_genes('ATP4A',genes_expr,group_list)
check_diff_genes('ALG1',genes_expr,group_list)
library(KEGGREST)
